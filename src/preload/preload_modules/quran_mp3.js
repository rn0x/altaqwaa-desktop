module.exports = function quran_mp3(fs, path, App_Path, settings) {
  let Quran_json = fs.readJsonSync(
    path.join(__dirname, "../../data/Quran.json")
  );
  let mp3quran_json = fs.readJsonSync(
    path.join(__dirname, "../../data/mp3quran.json")
  );
  ...
  let favoritesPath = path.join(App_Path, "./data/favorites.json");
  let favorites = {};
  try {
    if (fs.existsSync(favoritesPath)) {
      favorites = fs.readJsonSync(favoritesPath);
    } else {
      fs.writeJsonSync(favoritesPath, {});
    }
  } catch (error) {
    favorites = {};
    fs.writeJsonSync(favoritesPath, {});
  ...
  for (let item of mp3quran_json) {
    ...
    // First column: "القارئ" label
    let reader_label = document.createElement("p");
    reader_label.className = "reader-label";
    reader_label.innerText = "القارئ";
    reader.appendChild(reader_label);

    // Second column: Sheikh info
    reader.appendChild(reader_info);
    reader_info.className = "reader-info";
    reader_info.appendChild(Sheikh_name);
    Sheikh_name.id = "Sheikh_name";
    Sheikh_name.innerText = item?.name;
    reader_info.appendChild(Sheikh_rewaya);
    Sheikh_rewaya.id = "Sheikh_rewaya";
    Sheikh_rewaya.innerText = item?.rewaya;

    // Third column: favorite icon
    reader.appendChild(favorite_icon);
    favorite_icon.className = "favorite-icon";
    favorite_icon.id = `favorite_icon_${item?.id}`;
    favorite_icon.src = favorites[item?.id]
      ? dark_mode
        ? "../public/icon/dark/heart-solid.svg"
        : "../public/icon/heart-solid.svg"
      : dark_mode
      ? "../public/icon/dark/heart.svg"
      : "../public/icon/heart.svg";
    favorite_icon.title = favorites[item?.id]
      ? "إزالة من المفضلة"
      : "إضافة إلى المفضلة";
  }
  ...
  function toggleFavorite(sheikhId, event) {
    event.stopPropagation(); // Prevent li click event
    let favoritesPath = path.join(App_Path, "./data/favorites.json");
    let currentFavorites = fs.readJsonSync(favoritesPath);
    let favoriteIcon = document.getElementById(`favorite_icon_${sheikhId}`);

    if (currentFavorites[sheikhId]) {
      delete currentFavorites[sheikhId];
      favoriteIcon.src = dark_mode
        ? "../public/icon/dark/heart.svg"
        : "../public/icon/heart.svg";
      favoriteIcon.title = "إضافة إلى المفضلة";
    } else {
      currentFavorites[sheikhId] = true;
      favoriteIcon.src = dark_mode
        ? "../public/icon/dark/heart-solid.svg"
        : "../public/icon/heart-solid.svg";
      favoriteIcon.title = "إزالة من المفضلة";
    }

    fs.writeJsonSync(favoritesPath, currentFavorites);
    favorites = currentFavorites;

    // If showing only favorites, reapply filter
    if (showOnlyFavorites) {
      filterFavorites();
    }
  }

  // Add favorite icon click handlers
  mp3quran_json.forEach((item) => {
    let favoriteIcon = document.getElementById(`favorite_icon_${item?.id}`);
    if (favoriteIcon) {
  ...
  item_li_Sheikh.forEach((event, item) => {
    document.getElementById(event.id).addEventListener("click", (e) => {
      ...
      let ArrayMp3 = mp3quran_json.find(
        (e) => e?.id.toString() === Sheikh_true
      );
      let quran_mp3_name_Sheikh = document.getElementById(
        "quran_mp3_name_Sheikh"
      );
      ...
      for (let item of Quran_json) {
        ...
        icon_mp3.src = dark_mode
          ? "../public/icon/play.png"
          : "../public/icon/dark/play.png";
        ...
        icon_mp3_ddownload.src = dark_mode
          ? "../public/icon/download.png"
          : "../public/icon/dark/download.png";
      ...
      item_icon_mp3.forEach((event) => {
        ...
        icon_mp3.addEventListener("click", async (event) => {
          let sound_json = fs.readJsonSync(
            path.join(App_Path, "./data/sound.json")
          );
          let ArrayMp3 = mp3quran_json.find(
            (e) => e?.id.toString() === Sheikh_true
          );
          ...
          if (sound_json?.sound) {
            icon_mp3.src = dark_mode
              ? "../public/icon/pause.png"
              : "../public/icon/dark/pause.png";
            sound.play();
            fs.writeJsonSync(path.join(App_Path, "./data/sound.json"), {
              sound: false,
            });
          } else {
            icon_mp3.src = dark_mode
  ...
  searchInput.addEventListener("input", function () {
    ...
    sheikhs.forEach(function (li) {
      ...
      let sheikhId = li.getAttribute("data-sheikh-id");

      let matchesSearch = sheikhName.includes(searchValue);
  ...
  // Favorite filter button logic
  let favoriteFilterBtn = document.getElementById("favorite-filter-btn");
  ...
  favoriteFilterBtn.addEventListener("click", function () {
    ...
    if (showOnlyFavorites) {
      ...
      favoriteFilterBtn.title = "عرض الكل";
    } else {
      favoriteFilterBtn.classList.remove("active");
      favoriteFilterBtn.title = "عرض المفضلة";
    }
